 ...
 *
 * 这里曾经还有第三段「一键安装 / 刷新接线」（宿主路由 /dsh-extras/api/*），已按需移除：
 * 同一职责改由**启动器自愈**承担 —— dsh-tray.ps1 与 launch-dsh-web.cmd 在起 dsh 之前会先跑
 * 一次 scripts/install.ps1（幂等、失败不阻塞启动）。所以这一页现在是两段，别照旧版加回来。
 *
 * 为什么两段都写在这一个 bundle 里：设置页里要「一页两段」，而手写的 loader bundle
	...
	factory: (require) => {
		...
		function ShutdownBlock() {
			...
			return h(Block, {
				...
				blurb: '优雅关闭当前 DSH 进程，之后需要手动重新启动。',
				children,
			});
		}
		...
		// 页面与两块各自导出，方便冒烟测试逐块跑状态机（浏览器里不好调试）。
