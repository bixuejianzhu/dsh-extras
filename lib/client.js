...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const INSTALL_STATUS = '/dsh-extras/api/status';
		const INSTALL_GO = '/dsh-extras/api/install';
		...
		const S = {
			...
			pre: {
				margin: 0, padding: 10, maxHeight: 220, overflow: 'auto',
				fontSize: 11.5, lineHeight: 1.5, whiteSpace: 'pre-wrap', wordBreak: 'break-all',
				border: '1px solid var(--dsw-alias-border-l1, #3a3a42)', borderRadius: 8,
				background: 'var(--dsw-alias-bg-layer-2, rgba(0,0,0,0.18))',
		...
		function RestartBlock() {
			...
			return h(Block, {
				...
				blurb: '重启当前 DSH 进程（复用 dsh-market 的受保护重启通道），重启完成后本页会自动刷新。',
				children,
			});
		}
		...
		/**
		 * 第三段：一键安装 / 刷新接线。
		 * 状态机：checking -> idle -> confirm -> running -> done（或 error）
		 */
		function InstallBlock() {
			...
			react.useEffect(() => {
				let alive = true;
				readStatus(INSTALL_STATUS).then((body) => {
					if (!alive) return;
					setStatus(body);
					setPhase('idle');
				});
			...
			const run = async () => {
				setPhase('running');
				setDetail('正在运行安装器：复制/刷新 preset、重建 junction、更新 profile 清单…');
				setOutput('');
				try {
					const response = await fetch(INSTALL_GO, {
						method: 'POST',
						headers: { accept: 'application/json' },
					});
					const body = await readJson(response);
					if (body === null) {
						setPhase('error');
						setDetail(`安装路由返回 HTTP ${response.status}`);
						return;
					}
					setOutput(typeof body.output === 'string' ? body.output : '');
					if (body.ok !== true) {
						setPhase('error');
						setDetail(String(body.reason ?? body.hint ?? `HTTP ${response.status}`));
						return;
					}
					setPhase('done');
					setDetail(String(body.hint ?? '安装器已跑完，重启 dsh 后生效。'));
				} catch (error) {
					const message = error !== null && error !== undefined && error.message
						? error.message
						: String(error);
					setPhase('error');
					setDetail(`请求安装器失败：${message}`);
				}
			};

			const broken = status !== null && (status.failed !== undefined || status.installerExists !== true);
			const last = status !== null && status.lastRun !== null && status.lastRun !== undefined ? status.lastRun : null;
			const children = [];

			if (phase === 'checking') {
				children.push(h('p', { key: 'checking', style: S.muted }, '正在检查安装器…'));
			} else if (broken) {
				children.push(h('div', { key: 'broken', style: S.error },
					`安装器不可用：${status.failed ?? `找不到 ${status.installer}`}`));
			} else if (phase === 'idle') {
				if (last !== null) {
					children.push(h('p', { key: 'last', style: S.muted },
						`上次运行：${last.at}（${last.ok ? '成功' : '失败'}）`));
				}
				children.push(h('p', { key: 'where', style: S.muted },
					`安装器：${status.installer}`));
				children.push(h('div', { key: 'row', style: S.row },
					Button({ style: S.btnPrimary, label: '运行安装器', onClick: () => setPhase('confirm') })));
			} else if (phase === 'confirm') {
				children.push(h('p', { key: 'ask', style: S.desc },
					'安装器会重写 profile 的清单与 junction，并重新生成用户 preset（会先备份 profile 补丁层）。'
					+ '它不联网、不装包，可以反复运行。确定现在运行吗？'));
				children.push(h('div', { key: 'row', style: S.row }, [
					Button({ key: 'go', style: S.btnConfirm, label: '确认运行', onClick: () => { void run(); } }),
					Button({ key: 'no', style: S.btn, label: '取消', onClick: () => setPhase('idle') }),
				]));
			} else if (phase === 'running') {
				children.push(h('div', { key: 'row', style: S.row },
					Button({ style: S.btnPrimary, label: '正在运行…', disabled: true, onClick: () => {} })));
			} else if (phase === 'done') {
				children.push(h('div', { key: 'ok', style: S.ok }, detail));
				children.push(h('p', { key: 'next', style: S.muted },
					'下一步：用上面的「重启 DSH」，或者关掉 DSH 后重新启动一次。'));
			} else if (phase === 'error') {
				children.push(h('div', { key: 'error', style: S.error }, `安装失败：${detail}`));
			}
			...
			if (output !== '') {
				children.push(h('pre', { key: 'out', style: S.pre }, output));
			}
			if (phase === 'error') {
				children.push(h('div', { key: 'row', style: S.row },
					Button({ style: S.btn, label: '返回', onClick: () => { setPhase('checking'); setDetail(''); setAttempt((n) => n + 1); } })));
			}

			return h(Block, {
				heading: [h('span', { key: 'i', style: { display: 'inline-flex', marginRight: 6, verticalAlign: '-2px' } }, Glyph('install', 14)), '安装 / 刷新接线'],
				blurb: '一键重跑插件组的安装器：重新生成用户 preset、重建三个成员的 junction 与 profile 清单。'
					+ '新机器上第一次仍需先跑一次命令或双击组目录里的「安装.cmd」—— 这个按钮本身要靠插件组已挂载才存在。',
				children,
			});
		}
